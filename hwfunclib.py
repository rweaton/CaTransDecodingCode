...
def PeriEventTimeHistogramClassifier(DataDict, ParamsDict):
    
    BoundaryWindow = ParamsDict['BoundaryWindow'];
    Resolution = ParamsDict['Resolution'];
    TargetBinWidth = ParamsDict['TargetBinWidth'];
    
    # Read in event names to a list
    EventsList = list(DataDict['events'].keys())
    
    # Read in neuron names to a list
    NeuronsList = list(DataDict['neurons'].keys())    
     
    # Initialize dict of dicts to contain peri-event spike activity
    PeriEventActivityDicts = defaultdict(dict)

    EventIndices = np.arange(0, len(EventsList))
    NeuronIndices = np.arange(0, len(NeuronsList))
    
    # For each neuron for each event, extract spike activity about event time.
    for Event in EventsList:
        
        for Neuron in NeuronsList:
            
            # Call peri-event extraction routine
            PeriEventTimesArray = PeriEventTimeStampExtractor(
                    DataDict['events'][Event], 
                    DataDict['neurons'][Neuron], 
                    BoundaryWindow)
            
            # Compile spike raster with bin width of one resolution
            RasterDict0 = RasterMatrixCompiler(PeriEventTimesArray, 
                                              BoundaryWindow, 
                                              Resolution)
            
            # Widen resolution of raster bins to TargetBinWidth
            RasterDict = RasterBinWidener(RasterDict0, TargetBinWidth)
            
            # Compile histogram
 #           PE_HistoDict = PeriEventHistoFromRaster2(RasterDict0, TargetBinWidth)
            
#            RowIndex = EventIndices[np.array(EventsList) == Event][0]
#            ColumnIndex = NeuronIndices[np.array(NeuronsList) == Neuron][0]
            
            # Read in raster to appropriate element in dictionary
            PeriEventActivityDicts[Event][Neuron] = RasterDict['RasterMatrix_b']
    
    # Format peri-event activity into column vectors of length [nTrials*nBins] with
    # each column corresponding to each neuron. Dimensions [nTrials*nBins X nNeurons].
    # NOTE: Here, each column contains data pooled from ALL event types; that is:
    # nTrials =  nTrials_ev1 + nTrials_ev2 + ...
    InputColumns = zScoreInputFormatter2(PeriEventActivityDicts)
    
    # Determine number of trials for each event type and save to an array.
    nTrialsPerEvent = np.empty((len(EventsList),))
    
    # Determine number of PCs and total number of trials.
    (nTotalTrials, nNeurons) = InputColumns.shape
    
    # Count the number of trials performed for each event type.
    for i in np.arange(0, nTrialsPerEvent.size):
        
        (nTrialsPerEvent[i], nBins) = PeriEventActivityDicts[
                                        EventsList[i]][NeuronsList[0]].shape        
        
    # Arrange projections array back into the original "PSTH input" format of
    # dimension [nTrials X nBins*nNeurons].  PEA means "peri-event activity".
    # Each row is a flattened matrix of trial activity to be used for 
    # classification.
    PEA_Array = OutputFormatter(InputColumns, nBins)
    
    # Generate a dictionary of lists of indices for referencing trials for each
    # event.
    TrialIndicesByEventDict = defaultdict()
    nTrialsPerEvent = np.array(nTrialsPerEvent, dtype=int)
    CumSumTrialsPerEvent = np.cumsum(nTrialsPerEvent)
    
    for i in np.arange(0, nTrialsPerEvent.size):
        
        if i == 0:
            
            TrialIndicesByEventDict[EventsList[i]] = np.arange(0, 
                                   CumSumTrialsPerEvent[i])
            
        else:
            
            TrialIndicesByEventDict[EventsList[i]] = np.arange(CumSumTrialsPerEvent[i - 1], 
                                   CumSumTrialsPerEvent[i])
            
    # Initialize confusion matrix
    ConfusionMatrix = np.zeros((len(EventsList), len(EventsList)), dtype=int)
    
    # Initialize dict to contain the PSTH templates from all trials for each event
    WholeTemplatesByEventDict = defaultdict()
    
    # Initialize array to contain Euclidean distances
    #EventIndices = np.arange(0, EventsList.size)
    d_Array = np.empty((len(EventsList),))
    d_ArrayIndices = np.arange(0, len(EventsList))
    
    # Compile each of the complete templates
    for Event in EventsList:
        
        WholeTemplatesByEventDict[Event] = MakeTemplate(PEA_Array[
                TrialIndicesByEventDict[Event]])    
    
    # Iterate through each single trial in the peri-event activity array.  Find
    # the event type to which the current trial belongs and compute the adjusted
    # template.
            
    (nTotalTrials, nTotalFeatures) = PEA_Array.shape
    
    for TestTrialIndex in np.arange(0, nTotalTrials):
        ...
        # Compute the adjusted template
        TestActivity, AdjustedTemplate = \
            LeaveOneOutParser(PEA_Array, TrialIndicesByEventDict[CurrentEvent], 
                              TestTrialIndex)    
        for e in EventIndices:
            
            if CurrentEvent == EventsList[e]:
            
                d_Array[e] = EuclideanDistanceCalculator(TestActivity, 
                       AdjustedTemplate)
            else:
                
                d_Array[e] = EuclideanDistanceCalculator(TestActivity, 
                       WholeTemplatesByEventDict[EventsList[e]])
                
        #PredictedEventIndex = np.argmin(d_Array)
        PredictedEventIndex = d_ArrayIndices[d_Array == np.amin(d_Array)]
        
        if PredictedEventIndex.size > 1:
            
            print('multiple minima detected')
        
        TrueEventIndex = EventIndices[EventsList == np.array(CurrentEvent)][0]
        
        ConfusionMatrix[TrueEventIndex, PredictedEventIndex] += 1
        
    OutputDict = defaultdict(dict)
    
    OutputDict['confusion_matrix'] = ConfusionMatrix.tolist()
    OutputDict['performance'] = PerformanceCalculator(ConfusionMatrix)
    OutputDict['mutual_info'] = MutualInformationFromConfusionMatrix(ConfusionMatrix)
    
    return OutputDict, WholeTemplatesByEventDict, PEA_Array, InputColumns

def TemplatesDictChecker(TemplatesDict1, TemplatesDict2):
    
    BinsList = list(TemplatesDict1.keys())
    EventsList = list(TemplatesDict1[BinsList[0]].keys())
    
    OutputDict = defaultdict(dict)
    
    for BinEntry in BinsList:
        
        for EventEntry in EventsList:
            
            OutputDict[BinEntry][EventEntry] = np.sum(
                    TemplatesDict1[BinEntry][EventEntry] != 
                    TemplatesDict2[BinEntry][EventEntry])
            
    return OutputDict

def DataDictChecker(DataDict1, DataDict2):
    
    EventsList = list(DataDict1['events'].keys())
    NeuronsList = list(DataDict1['neurons'].keys())
    
    OutputDict = defaultdict(dict)
    
    for EventEntry in EventsList:
        
        OutputDict['events'][EventEntry] = np.sum(
                    DataDict1['events'][EventEntry] != 
                    DataDict2['events'][EventEntry])
        
    for NeuronEntry in NeuronsList:
        
        OutputDict['neurons'][NeuronEntry] = np.sum(
                    DataDict1['neurons'][NeuronEntry] != 
                    DataDict2['neurons'][NeuronEntry])
        
    return OutputDict

def PEA_ArraysDictChecker(PEA1, PEA2):
    
    BinsList = list(PEA1.keys())
        
    OutputDict = defaultdict(dict)
    
    for BinEntry in BinsList:
        
        OutputDict[BinEntry] = (PEA1[BinEntry] != PEA2[BinEntry])

    return OutputDict

def InputColumnsDictChecker(InColDict1, InColDict2):
    
    BinsList = list(InColDict1.keys())
        
    OutputDict = defaultdict(dict)
    
    for BinEntry in BinsList:
        
        OutputDict[BinEntry] = (InColDict1[BinEntry] != InColDict2[BinEntry])
